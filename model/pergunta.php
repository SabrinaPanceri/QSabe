...
//        }
    ...
    function buscatodasperguntas(){
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            $result2 = $this->banco->executequery($sql2);
            $row2 = mysqli_fetch_assoc($result2);
            $html = '<div class="post"> <div class="entry"><p>#'.$row["idpergunta"].' <a href="/qsabe/view/view_pergunta.php?pergunta='.$row["idpergunta"].'">'.$row["desc_pergunta"].'</a></p></br></br> by:'.$row2["nome_exibicao"].' date:'.$row["data_reg"].' </div> </div>';
            $html_geral= $html_geral.$html;
        }
        return $html_geral;
